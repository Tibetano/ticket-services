    ...
    public CardResponse chargeCard(Customer customer, OrderEntity order, CreditCardDTO card) {
        ...
        try {
            ...
            JSONObject response = efiPay.call(
                    "createOneStepCharge",
                    Map.of(),
                    creditCardMapper.generateRequestBody(customer.getFullName(), customer.getCpf(),
                            customer.getPhoneNumber(), customer.getEmail(), card.installments(),
                            card.paymentToken(), itemList)
