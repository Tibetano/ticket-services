...
CREATE TYPE payment_status AS ENUM ('PENDING', 'APPROVED', 'FAILED', 'CANCELED', 'REFUNDED');
CREATE TYPE payment_method AS ENUM ('PIX', 'BOLETO', 'CREDIT_CARD');


CREATE TYPE fee_payer AS ENUM ('CUSTOMER', 'MERCHANT');
...
CREATE TABLE ticket_services.orders (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    order_number    BIGINT GENERATED ALWAYS AS IDENTITY NOT NULL, -- foi utilizado "ALWAYS" ao invés do "BY DEFAULT"("ADD COLUMN numero_pedido BIGINT GENERATED BY DEFAULT AS IDENTITY UNIQUE") pq com always o controle fica 100% com o banco, ou seja, não é permitida a inserção manual desse atributo (da pra forçar a inserção mas exige comandos chatos, se for o caso, busque como forçar a inserção...).
...
CREATE INDEX idx_orders_customer_id ON ticket_services.orders(customer_id);
CREATE INDEX idx_orders_status ON ticket_services.orders(status);
CREATE INDEX idx_orders_expires_at ON ticket_services.orders(expires_at);
...
CREATE TABLE ticket_services.ticket (
    ...
    status                  ticket_status NOT NULL DEFAULT 'VALID', --esse atributo será utilizado para invalidar o ingresso assim que ele for apresentado na entrada do evento.
...
CREATE TABLE ticket_services.payment (
    ...
    amount                      INTEGER NOT NULL, -- valor total pago pelo cliente (em centavos)

    provider_fee_fixed          INTEGER NOT NULL DEFAULT 0, -- taxa fixa do gateway/banco
    provider_fee_percentage     NUMERIC(5,2), -- porcentagem aplicada pelo gateway
    provider_fee_amount         INTEGER NOT NULL DEFAULT 0, -- valor total da taxa cobrada

    net_amount                  INTEGER GENERATED ALWAYS AS (
                                    amount - provider_fee_amount
                                ) STORED, -- valor líquido recebido

    fee_payer                   fee_payer NOT NULL DEFAULT 'MERCHANT', -- campo para indicar quem absorveu a taxa (cliente ou evento)
    ...
    CONSTRAINT chk_provider_fee_fixed_positive CHECK (provider_fee_fixed >= 0),

    CONSTRAINT chk_provider_fee_amount_positive CHECK (provider_fee_amount >= 0),

    CONSTRAINT chk_provider_fee_percentage_positive CHECK (
            provider_fee_percentage IS NULL OR provider_fee_percentage >= 0),
...
CREATE INDEX idx_payment_provider_charge_id ON ticket_services.payment(provider_charge_id);
